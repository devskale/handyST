...
pub struct AudioRecorder {
    ...
    /// Silence auto-stop (sttts): fire after this much VAD silence following
    /// speech. 0 = off.
    auto_stop_ms: u64,
...
/// Returns true when the frame counted as speech (sttts silence auto-stop).
fn handle_frame(
    samples: &[f32],
    vad_policy: VadPolicy,
    vad: &Option<VadConfig>,
    ...
    if vad_policy == VadPolicy::Disabled {
        emit(samples);
        // No VAD: treat everything as speech so auto-stop never fires.
        return true;
    }
...
struct CaptureProcessor {
    ...
    // ---- sttts: silence auto-stop --------------------------------------- //
    /// Fire the auto-stop callback after this much VAD silence following
    /// speech. 0 = off.
    auto_stop_ms: u64,
    auto_stop_cb: Option<Arc<dyn Fn() + Send + Sync + 'static>>,
    speech_seen: bool,
...
impl CaptureProcessor {
    ...
    fn process_raw_chunk(&mut self, raw: &[f32], disposition: ChunkDisposition) {
        ...
        self.frame_resampler.push(raw, |frame: &[f32]| {
            ...
            // sttts silence auto-stop: count VAD silence after speech.
            if self.auto_stop_ms > 0 && !self.auto_stop_fired {
                if is_speech {
                    ...
                    if self.silence_samples
                        >= self.auto_stop_ms as usize * constants::WHISPER_SAMPLE_RATE as usize
                            / 1000
                    {
                        self.auto_stop_fired = true;
                        log::info!("auto-stop: {}ms of silence after speech", self.auto_stop_ms);
