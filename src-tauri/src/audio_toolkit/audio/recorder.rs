...
/// sttts: accumulate VAD silence after speech; fire the auto-stop callback
...
fn track_auto_stop(
    ...
    speech_seen: &mut bool,
    silence_samples: &mut usize,
    auto_stop_fired: &mut bool,
    auto_stop_cb: &Option<Arc<dyn Fn() + Send + Sync + 'static>>,
) {
    if *auto_stop_fired {
        return;
...
fn run_consumer(
    ...
    // sttts silence auto-stop state
    let auto_stop_threshold_samples =
        (auto_stop_ms as usize) * crate::audio_toolkit::constants::WHISPER_SAMPLE_RATE as usize
            / 1000;
