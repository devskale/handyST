...
fn version_label() -> String {
    if cfg!(debug_assertions) {
        format!("handyST v{} (Dev)", env!("CARGO_PKG_VERSION"))
    } else {
        format!("handyST v{}", env!("CARGO_PKG_VERSION"))
    }
}
...
pub fn update_tray_menu(app: &AppHandle, locale: Option<&str>) {
    ...
    // sttts: no local models — the model submenu and "unload model" item are
    // gone; transcription is remote.
